...
![New](../assets/new.svg)<!-- Issue PAY-4050 -->为订单付款状态报告添加了[数据可视化视图](/help/payment-services/payouts.md#payouts-data-visualization-view)，该视图显示在付款服务主页上。
...
![新的](../assets/new.svg)<!-- Issue PAY-1720 -->商店订单争议现已在[订单付款状态报表](/help/payment-services/order-payment-status.md#view-disputes)中可用。 您可以直接从[!DNL Payment Services]导航到PayPal解决中心来解决争议。
...
![新](../assets/new.svg)<!-- Issue PAY-2761 -->您现在可以通过使用“列”设置控件显示或隐藏列，自定义在[订单付款状态报表](/help/payment-services/order-payment-status.md#show-and-hide-columns)和[付款报表](/help/payment-services/payouts.md#show-and-hide-columns)中显示的数据。
...
![新](../assets/new.svg)<!-- PAY-6227 -->使用PayPal结帐时，[!DNL Payment Services]将跳过订单确认弹出窗口，以加快购买流程。 这由[服务器端送货回调](payments-options.md#server-side-shipping-callback)启用，它直接在PayPal审核页面中计算送货和总计。
...
![新](../assets/new.svg)<!-- PAY-4877 -->现在，[!DNL Payment Services]支持[L2/L3定价功能](/help/payment-services/levels-card-payment-transactions.md#level-2-and-level-3)。 此功能仅适用于已启用IC++定价的[!DNL Payment Services]客户。 如果要使用L2/L3处理[!DNL Payment Services]的数据，请与您的[!DNL Payment Services]帐户管理员联系。
...
![新](../assets/new.svg)<!-- PAY-4868 -->商家可以通过管理员成功[在整个购买体验](/help/payment-services/payments-options.md)中配置Google Pay，类似于[!DNL Payment Services]中的其他付款按钮。
...
![修复](../assets/fix.svg)<!-- PAY-5291 -->某些交易记录的接收付款确认可能延迟。 在这种情况下，商家现在可以获得订单的更新付款状态。 [付款服务通过检测待定交易并主动监视这些交易并在捕获待定状态时进行更新，来检测订单中付款交易](/help/payment-services/order-payment-status.md#payment-status-updates)的待定状态。
...
![新](../assets/new.svg)<!-- Issue xxx -->在下列国家/地区和相关货币中添加了[可用性](introduction.md#availability)：澳大利亚、法国、英国。
...
![新](../assets/new.svg)<!-- Issue PAY-2682 --> [!DNL Payment Services]和[!DNL Adobe Commerce]的[!DNL Magento Open Source]扩展现在适用于加拿大商家。 商家可以使用[法语](introduction.md?lang=fr#accepted-credit-cards-and-currencies)或[英语](introduction.md#accepted-credit-cards-and-currencies)查看付款配置。
