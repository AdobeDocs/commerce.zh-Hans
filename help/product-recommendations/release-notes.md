...
发行说明描述了对以下[!DNL Product Recommendations]模块的更新：
...
[了解有关产品支持的更多信息](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/release/product-availability)。
...
![修复](../assets/fix.svg) （仅限&#x200B;**[!DNL Adobe Commerce as a Cloud Service]**）更新了[产品预览面板](create.md#preview-recommendations)：现在仅对需要SKU进行预览的推荐类型显示该面板（例如，交叉销售、追加销售和基于相似度的推荐）。 对于其他类型，预览是隐藏的。 不支持预览时，个性化类型（如&#x200B;**最近查看的项目**&#x200B;和&#x200B;**为您推荐的项目**）会显示消息。
...
![新建](../assets/new.svg)您现在可以为每种页面类型创建最多50个有效的推荐单位。 以前的上限为5个。
...
![修复](../assets/fix.svg)改进了产品推荐单元在不同环境中的交互方式。
...
![新](../assets/new.svg)添加了对在[产品推荐工作区](workspace.md)的非默认存储视图中创建的[PageBuilder推荐单元](page-builder.md)的量度的显示支持。
...
![错误](../assets/bug.svg) `ProductRecommendations Layout` XML文件中的`referenceBlock`元素导致验证错误。
...
![新](../assets/new.svg) [!DNL Product Recommendations]现在支持Commerce中的完整[Inventory management](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/inventory/introduction)功能（以前称为多Source清单或MSI）。 要启用完全支持，您必须[将](install-configure.md#update)依赖项模块`commerce-data-export`更新为版本102.2.0+。
