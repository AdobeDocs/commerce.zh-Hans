...
目录层允许您在不更改原始源数据的情况下修改产品数据。 图层通过在基本目录之上创建一个图层来修改产品属性，如名称、描述、图像、链接和元数据。 原始产品数据保持不变，允许您安全自定义产品并随时恢复更改。

![目录层](../assets/catalog-layers.png)

## 目录层的工作方式

当客户查看您的店面时，系统会将您的基本目录数据与活动目录层相结合，以显示最终产品信息。 此过程如下所示：
...
1. **字段处理** — 不同的字段类型处理方式不同：
   ...
   * **覆盖字段** — 名称、说明和元标题等文本字段将替换为图层值，优先级较高的图层优先。
...
当您启用[产品可视化图表](product-visuals.md)时，AEM Assets集成会自动创建和管理专门用于AEM Assets内容的目录层。 默认图层名称为`AEM-Assets`；但是，您可以在AEM Assets集成](../../aem-assets-integration/get-started/configure-aco.md)中的[新用户引导期间指定自定义名称。
...
>[!CAUTION]
>
> 将AEM-Assets层分配给目录视图。 如果未指定图层，则产品图像数据会意外被覆盖。
...
1. 导航到&#x200B;**[!UICONTROL Store setup]**，然后单击&#x200B;**[!UICONTROL Catalog views]**。
...
>[!NOTE]
>
>如果目录视图中不存在Sites Optimizer图层，则Sites Optimizer中的自动修复功能会自动创建一个图层，并为其分配最高优先级（最高编号）。 如果删除此图层，则会在下次运行Sites Optimizer中的自动修复功能时重新创建此图层，并将现有图层移动到较低顺序编号。 如果Sites Optimizer层的订单号不同，则自动修复功能不会更改其优先级。
...
1. 导航到&#x200B;**[!UICONTROL Store setup]** > **[!UICONTROL Catalog views]**。
...
1. 单击&#x200B;**[!UICONTROL Save]**&#x200B;以应用新的优先级顺序。

>[!IMPORTANT]
>
>对层优先级的更改会立即生效并影响客户在您店面看到的内容。 在保存之前查看预览，以确保应用正确的值（**预览尚不可用**）。

## 最佳实践

使用目录层时，请遵循以下建议：

* **使用描述性名称** — 清楚地命名图层以指示其用途（例如，“Holiday 2025 Campaign”或“SEO优化 — 产品页面”）。

* **限制层** — 虽然系统支持多个层，但使用太多可能会影响性能。 如果可能，合并层。


* **文档优先级逻辑** — 跟踪哪些图层优先，以避免意外覆盖。

* **查看Sites Optimizer图层** — 使用Sites Optimizer中的自动修复时，系统会创建具有最高优先级的图层。 在添加覆盖AI推荐的手动层时请务必小心。 了解有关使用[Sites Optimizer](../manage-results/opportunities.md)的更多信息。
...
* [受限访问密钥](restricted-access-keys.md) — 使用签名令牌身份验证保护目录视图
