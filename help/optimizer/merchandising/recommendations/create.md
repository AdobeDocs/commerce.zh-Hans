...
1. 使用&#x200B;_推荐的产品预览_&#x200B;面板可以更好地了解筛选器如何影响推荐单元中显示哪些产品。 详细了解如何[预览推荐](#preview-recommendations)。
...
>[!NOTE]
>
>您最多可以创建50个有效的推荐单位。 有关详细信息，请参阅[限制和边界](../../boundaries-limits.md)。
...
对于多店面、多语言或多品牌设置，您可以配置每个推荐是应用于所有目录视图（全局）还是应用于单个[目录视图](../../setup/catalog-view.md)。 了解有关在使用推荐时如何[设置目录视图](../../manage-results/recommendation-performance.md#select-catalog-view)的详细信息。
...
1. 在菜单中，您可以&#x200B;**停用**、**删除**&#x200B;或&#x200B;**编辑**&#x200B;推荐。 如果选择&#x200B;**编辑**，则可以根据需要调整以下设置：
   ...
   您无法更改推荐类型或目录视图。 目录视图是在创建推荐时设置的。 若要了解详细信息，请参阅[选择目录视图](../../manage-results/recommendation-performance.md#select-catalog-view)。

1. 完成后，单击&#x200B;**保存更改**。

## 就绪指示器

就绪指示器根据可用的目录和行为数据显示哪些推荐类型表现最佳。 它们还可以帮助您识别[事件集合](../../setup/events/overview.md)的潜在问题，或确定推荐类型是否接收不到足够的流量来生成结果。
...
由于这些变量，就绪指示器百分比可能会波动。 此波动解释了您可能会看到推荐类型进出“准备部署”的原因。

就绪指标的计算基于两个因素：

- 足够的结果集大小：在大多数情况下返回的结果是否足够以避免使用[备份建议](../../setup/events/overview.md#backup-recommendations)？
- 充分的结果集多样性：返回的产品是否代表您目录中的各种产品？ 此因素的目标是避免少数产品成为整个站点中唯一推荐的项目。

基于上述因素，就绪值计算并显示如下：

- 75%或更高的比例意味着针对该建议类型提出的建议具有高度相关性。
- 至少50%意味着针对该建议类型提出的建议不那么相关。
- 不足50%意味着为该建议类型提出的建议可能没有相关性。 在这种情况下，使用了[备份推荐](../../setup/events/overview.md#backup-recommendations)。

了解有关[为何就绪指示器可能较低的更多信息](#what-to-do-if-the-readiness-indicator-is-low)。
...
依赖目录数据的推荐类型的就绪指示器不会发生太大变化，因为商户的目录很少发生变化。 但是，基于购物者行为数据的推荐类型的就绪性指标可能会经常更改，具体取决于每日购物者活动。

#### 如果就绪指示器低，该怎么办

就绪性百分比低表示您的目录中符合此推荐类型推荐条件的产品不多。 这意味着，如果仍部署此推荐类型，则返回[备用推荐](../../setup/events/overview.md#backup-recommendations)的可能性很高。
