...
- **管理员** — 有三种不同类型的管理员角色：
   ...
   - [产品管理员](#add-a-product-admin) — 产品管理员可以在[中](#add-users)管理产品[!DNL Adobe Admin Console]的用户、角色和权限。
...
根据您的组织配置时间，有两种不同的方法将产品管理员用户添加到[!DNL Adobe Commerce Optimizer]。 在早期访问组织中，每个分配了产品管理员角色的用户都有权管理组织中的所有实例。 在2025年10月13日之后配置的通用可用性(GA)组织中，您可以将用户分配为特定实例的产品管理员。 当产品管理员用户登录时，他们只能看到自己有权管理的实例。
...
>[!NOTE]
>
>只有产品管理员和系统管理员可以将用户和开发人员添加到[!DNL Adobe Commerce Optimizer]产品。
