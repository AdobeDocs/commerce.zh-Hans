...
根据您的组织配置时间，可通过两种不同的方式将产品管理员用户添加到Adobe Commerce Optimizer。 在早期访问组织中，每个分配了产品管理员角色的用户都有权管理组织中的所有实例。 在2025年10月13日之后配置的通用可用性(GA)组织中，您可以将用户分配为特定实例的产品管理员。 当产品管理员用户登录时，他们只能看到自己有权管理的实例。
