...
▢生产实例处于活动状态。请参阅[入门](../get-started.md)，了解如何配置它。
▢实例位于正确的区域。
▢环境类型为“生产”。
...
▢ `aco:conf:show` CLI命令用于确认与Commerce Optimizer生产实例的连接。组织ID、客户端ID、摄取URL和Commerce Optimizer URL与生产环境相匹配。
[导出配置](../../aco-connector/get-started.md)中的▢同步作用域符合您的要求。
▢ [数据馈送同步状态](../../aco-connector/data-sync-manage.md)确认从云实例导出数据。
...
▢ [促销规则](../merchandising/rules/overview.md)已配置。
▢ [价格分面和搜索语言](../settings.md)在您使用这些功能时符合您的要求。
▢ [产品推荐](../merchandising/recommendations/create.md)存在并且行为与预览中预期的一样。
...
▢ `commerce-endpoint`在您使用API网格时指向生产Commerce Optimizer GraphQL端点或API网格端点。
▢ `headers.cs.AC-view-ID`包含您生产Commerce Optimizer实例中的目录视图ID。
...
▢ **配送：**&#x200B;履行平台已连接并测试（例如，ShipStation）。
▢ **税：**&#x200B;已验证税计算集成（Avalara、TaxJar等）。
▢ **税费：**&#x200B;会计软件同步工作（QuickBooks等）。
▢ **库存：** PIM、ERP或库存管理集成已测试并同步。
▢ **架构：**&#x200B;主机商务系统处理付款、运费、税收和库存（不是[!DNL Adobe Commerce Optimizer]）。
▢ **架构：** API Mesh和App Builder在主机商务系统和[!DNL Adobe Commerce Optimizer]之间保持同步。
▢ **电子邮件：**&#x200B;事务性电子邮件投放工作（订单确认、配送等）。
...
▢关键页（尤其是PDP和PLP）存在文档标题元数据。请参阅&#x200B;_Adobe Commerce店面_&#x200B;文档中的[SEO元数据](https://experienceleague.adobe.com/developer/commerce/storefront/setup/seo/metadata/){target="_blank"}。
▢个PDP包含[元数据和结构化数据](https://experienceleague.adobe.com/developer/commerce/storefront/setup/seo/metadata/){target="_blank"}（例如，JSON-LD）。
▢产品URL格式一致（例如，`domain/product-name`）。
▢个虚URL重定向到规范URL。
▢项目包括`robots.txt`，它允许在适当时进行索引、引用Sitemap并阻止您不希望进行索引的路径（例如，`/drafts`）。
▢ [重定向](https://www.aem.live/docs/redirects)文件涵盖迁移中的URL更改（例如，删除`.html`后）。
▢ Sitemap存在并根据需要提交到Google搜索控制台。
▢规范URL返回`2xx`状态（不是`3xx`或`4xx`）。
▢多语言站点在站点地图中包含`hreflang`标记。
▢ Google Search Console覆盖范围报告为最新。

### 预渲染

确认启用服务器端渲染的位置。

为关键页面启用了▢预呈现。请参阅&#x200B;_AEM店面_&#x200B;文档中的[Adobe Commerce的预呈现](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/aem-prerender/){target="_blank"}。
...
▢您的店面遵循&#x200B;_Adobe Commerce店面_&#x200B;文档中的[性能最佳实践](https://experienceleague.adobe.com/developer/commerce/storefront/get-started/performance/){target="_blank"}。
▢（可选）已配置Google Analytics和Google Tag Manager。
▢ [Storefront事件](https://github.com/adobe/commerce-events/tree/main/examples/events/snowplow-debugger)实现有效，并且数据显示在Adobe Commerce *管理员*&#x200B;的[!DNL Live Search]和[!DNL Product Recommendations]仪表板中。
▢ [Commerce配置](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/){target="_blank"}中的`environment`分析参数在开发期间为`"Testing"`，在上线时为`"Production"`。请参阅[分析检测](https://experienceleague.adobe.com/developer/commerce/storefront/setup/analytics/instrumentation/){target="_blank"}。
鉴于本主题中的指导，▢灯塔分数达到了您的目标（例如，关键页面上的`100`）。
...
▢推送失效工作可端到端地完成（发布更改，然后在生产域上验证）。
在直接转换之前，▢个DNS TTL足够低。
所有域和主机名的▢ DNS A和CNAME记录均正确。
▢已为生产域配置并验证SSL/TLS证书。
▢ `www`和Apex重定向行为正确。
...
已在站点范围强制使用▢ **SSL：** HTTPS。
▢ **访问：**&#x200B;默认&#x200B;*管理员*&#x200B;密码已更改并且已设置强密码策略。请参阅[!DNL Adobe Commerce Optimizer] [用户和身份管理](../user-management.md)。
▢ **访问：** *管理员* URL不是默认值。
已为所有*管理员*&#x200B;用户启用▢ **访问：**&#x200B;双重身份验证。
▢ **访问：**&#x200B;没有非活动或未使用的管理员用户与项目相关联。
▢ **防火墙：** Web应用程序防火墙(WAF)已配置并验证。
▢ **PCI：**&#x200B;生产环境的安全渗透测试（PCI范围）已完成。
▢ **扫描：** Adobe安全扫描工具已注册，初始扫描已完成。
▢ **访问：** CORS仅允许批准的源。
▢ **合规性：** [!DNL Adobe Commerce Optimizer]的[共享责任模型](../shared-responsibility.md)是最新的，并清楚地定义了Adobe与客户责任的对比。
...
▢ **性能：**&#x200B;负载和压力测试已完成，Adobe CTA或CSE得出了结果。
▢ **性能：**&#x200B;桌面和移动设备上的页面加载时间少于三秒。
▢ **性能：** Lighthouse得分满足关键页面上的目标（例如，通过PageSpeed Insights）。
已优化▢ **性能：**&#x200B;图像、脚本和资产。
...
▢ **启动后：**&#x200B;监控Google搜索控制台以获取索引并抓取错误。
▢ **启动后：**&#x200B;监视404报告并为高流量旧版URL添加重定向。
▢ **发布后：**&#x200B;确认MarTech和分析生产数据。
▢ **启动后：**&#x200B;请咨询您的CTA、CSE或AM以启用高SLA监控。
▢灾难恢复计划存在并通过测试。
▢已制定流程来跟踪样板和扩展包并将其升级到当前版本。
