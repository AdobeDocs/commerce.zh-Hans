...
- 了解如何使用[!DNL Adobe Commerce Optimizer]目录视图和策略来创建自定义目录视图和数据访问过滤器，并将数据发送到由Edge Delivery提供支持的Adobe Commerce店面。
...
导航到预配置了示例数据的Commerce Optimizer应用程序的URL。 您可以从Commerce Cloud项目的实例详细信息中找到Commerce Optimizer管理器中的URL，也可以从系统管理员处获取该URL。 （请参阅[访问实例](../get-started.md#access-the-adobe-commerce-optimizer-application)。）
...
1. 在左边栏中，展开&#x200B;_存储设置_&#x200B;部分，然后单击&#x200B;**[!UICONTROL Catalog views]**。
   ...
   请注意现有的目录视图： *Arkbridge*、*Kingsbluff*&#x200B;和&#x200B;*所有视图*。
...
本教程的最后一段涉及更新[您已经创建的](#prerequisites)店面，以将数据交付到新的Celport目录。 在此部分中，您将storefront配置文件中的目录视图ID替换为Celport的目录视图ID。
...
1. 尝试更新店面配置文件(`config.json`)。
   ...
   这些实验展示了[!DNL Adobe Commerce Optimizer]的灵活性 — 您可以快速在不同的目录视图和价格手册之间切换，为不同的受众创建自定义购物体验，而无需复制您的目录数据。
...
- **解决方案：**&#x200B;验证价格手册ID是否与您[!DNL Adobe Commerce Optimizer]实例中可用的价格手册ID匹配

**问题：**&#x200B;搜索未返回任何结果

- **解决方案：**&#x200B;确认目录视图策略允许搜索的产品类别

有关其他帮助，请参阅[[!DNL Adobe Commerce Optimizer] 文档](../overview.md)或与Adobe支持部门联系。
...
要继续了解[!DNL Adobe Commerce Optimizer]，请执行以下操作：
