...
>[!NOTE]
>
>`saas:resync`命令还适用于[!DNL Adobe Commerce Optimizer Connector]源，如`products`、`categories`和`priceBooks`。 有关连接器馈送和索引器名称的完整列表，请参阅[支持的馈送](../aco-connector/reference/connector-reference.md#supported-feeds)。
...
>[!NOTE]
>
>与标准重新同步不同，`--by-ids`绕过哈希验证并强制将指定的实体提交到连接的Commerce服务，无论其内容在上次导出后是否发生更改。
...
强制重新同步馈送项目，而不考虑其数据是否已更改，而不清除馈送导出状态。

>[!NOTE]
>
>需要[数据导出扩展](manage-extension.md#update-a-module-to-a-specific-version)的版本103.4.30或更高版本。
