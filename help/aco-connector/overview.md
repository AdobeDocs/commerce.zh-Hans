...
* **将Commerce目录数据导出到[!DNL Adobe Commerce Optimizer]** — 在网站和客户组级别导出价格和价格手册数据。 产品和产品属性数据在`store view`级别导出。 默认情况下，所有Commerce作用域（网站和商店视图）的目录数据同步处于启用状态。
  ...
  要启用此工作流，请安装`adobe-commerce/commerce-data-export-aco-adapter` PHP扩展，检查导出器配置，然后从Commerce管理员中启用Commerce与Commerce Optimizer之间的集成。 有关详细说明，请参阅[开始使用](get-started.md)。
