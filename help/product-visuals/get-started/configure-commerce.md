...
description: 了解如何在Adobe Commerce实例上安装适用于Adobe Commerce的AEM Assets集成。 要将产品可视化与Adobe Commerce结合使用，需要此扩展。
feature: CMS, Media
...
在版本为Adobe Commerce 2.4.5+的AEM Assets实例上安装最新版本的Adobe Commerce集成扩展(`aem-assets-integration`) for Product Visuals。 该扩展是作为[repo.magento.com](https://repo.magento.com/admin/dashboard)存储库中的编辑器中继包提供的。
