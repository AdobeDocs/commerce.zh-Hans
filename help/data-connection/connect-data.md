...
如果您计划收集和发送[历史订单数据](#send-historical-order-data)或[客户配置文件数据](#send-customer-profile-data)，则必须添加服务帐户和凭据详细信息。 此外，如果您正在配置[Audience Activation](https://experienceleague.adobe.com/en/docs/commerce-admin/customers/audience-activation)扩展，则必须完成这些步骤。
...
要创建项目，请按照[身份验证和访问Experience Platform API](https://experienceleague.adobe.com/en/docs/experience-platform/landing/platform-apis/api-authentication)教程中所述的步骤进行操作。

在学习本教程时，请确保您的项目具备以下条件：

- 访问以下[产品配置文件](https://experienceleague.adobe.com/en/docs/experience-platform/landing/platform-apis/api-authentication#select-product-profiles)： **默认的生产所有访问**&#x200B;和&#x200B;**AEP默认所有访问**。
- 已配置正确的[角色和权限](https://experienceleague.adobe.com/en/docs/experience-platform/landing/platform-apis/api-authentication#assign-api-to-a-role)。
...
1. （可选）如果您已将[AEP Web SDK (alloy)](https://experienceleague.adobe.com/en/docs/experience-platform/collection/js/js-overview)部署到您的网站，请启用该复选框并添加AEP Web SDK的名称。 否则，请将这些字段留空，[!DNL Data Connection]扩展将为您部署一个字段。
...
1. 如果要发送订单状态信息（如订单已下达、已取消、已退款或已发货），请选择&#x200B;**后台事件**。
   ...
   >[!NOTE]
   >
   >如果选择&#x200B;**后台事件**，则所有后台数据都会发送到Experience Platform Edge。 如果购物者选择退出数据收集，您必须在Experience Platform中明确设置购物者的隐私偏好设置。 这与店面事件不同，店面事件收集器已根据购物者偏好处理同意。 了解[更多](https://experienceleague.adobe.com/en/docs/experience-platform/landing/governance-privacy-security/consent/adobe/dataset)有关在Experience Platform中设置购物者的隐私首选项的信息。

1. （如果您使用自己的AEP Web SDK，请跳过此步骤。） [在Adobe Experience Platform中创建](https://experienceleague.adobe.com/en/docs/experience-platform/datastreams/configure#create)数据流或选择要用于收集的现有数据流。 在&#x200B;**数据流ID**&#x200B;字段中输入该数据流ID。
...
1. 若要确保根据[cron](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/tools/cron)作业按计划更新后台事件数据，必须将`Sales Orders Feed`索引更改为`Update by Schedule`。
   ...
   1. 如果您是首次启用后台数据，请运行以下命令来重新索引并触发重新同步。 只要[cron](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/tools/cron)作业设置正确，后续的重新同步就会自动进行。
...
配置文件记录包含购物者在Commerce实例中创建配置文件时保存的数据，例如购物者的姓名。 如果您的架构和数据集配置正确[&#128279;](profile-data.md)，则会将配置文件记录发送到Experience Platform并转发到Adobe的配置文件管理和分段服务： [Real-Time CDP](https://experienceleague.adobe.com/en/docs/experience-platform/rtcdp/intro/rtcdp-intro/overview)。
...
1. [启用](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/service/rabbitmq) RabbitMQ。
...
1. 使用`CRON_CONSUMERS_RUNNER`环境变量在`.magento.env.yaml`中通过cron作业启用消息队列使用者。
   ...
   >[!NOTE]
   >
   >请参阅[部署变量文档](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#cron_consumers_runner)以了解所有可用的配置选项。
...
要确认正在从Commerce存储中收集数据，请使用[Adobe Experience Platform Debugger](https://experienceleague.adobe.com/en/docs/experience-platform/debugger/home)来检查您的Commerce网站。 确认正在收集数据后，可以通过运行查询来验证您的店面和后台事件数据是否显示在边缘，该查询会返回您在配置期间创建的[数据集中的数据](overview.md#enable-extension)。
...
如果结果不符合预期，请打开您的数据集并查找任何失败的批量导入。 了解有关[批处理导入疑难解答的详细信息](https://experienceleague.adobe.com/en/docs/experience-platform/ingestion/batch/troubleshooting)。
