...
* 要了解如何在AEM Assets中添加或更新产品资源（元数据、SKU链接、审批），请参阅[默认的自动匹配](synchronize/default-match.md)。
* 有关类别图像或页面生成器内容，请参阅[手动资源选择](synchronize/asset-selector-integration.md)。
...
1. 打开&#x200B;**图像和视频**&#x200B;部分。
   ...
   要配置产品资源（将图像链接到SKU），请打开您的AEM Assets创作实例，然后从主视图中单击&#x200B;**Assets**。 有关元数据配置步骤，请参阅[默认自动匹配](synchronize/default-match.md)。

### 在AEM Assets中管理产品图像

要管理与产品相关的图像，所有更改必须直接在&#x200B;**AEM Assets**&#x200B;中进行。 此过程是完全自动的，可确保任何更改同步到Adobe Commerce而无需手动干预。

要了解如何将资源链接到AEM Assets中的产品（包括元数据配置和批准），请参阅以下主题：
...
有关同步时间的信息，请参阅[同步SLA](get-started/setup-synchronization.md#synchronization-sla)主题。
...
[!BADGE 仅限PaaS]{type=Informative tooltip="仅适用于云项目上的Adobe Commerce（Adobe管理的PaaS基础架构）。"}要使用&#x200B;**AEM资产选择器**&#x200B;嵌入图像，请确保用户具有所需的[权限和IMS身份验证](get-started/permissions.md)。
...
>[!TIP]
>
> [DA.live （文档创作）](https://experienceleague.adobe.com/en/tools/commerce-storefront/merchants/storefront-builder/#dalive-document-authoring){target=_blank}还提供了一个资产选择器以扩充数据。
...
在配置[AEM Asset Selector](synchronize/asset-selector-integration.md)并确保用户具有所需的[权限和IMS身份验证](get-started/permissions.md)之后，您可以将其用于向目录类别内容添加资源。
