...
- 结构良好的产品数据确保搜索算法能够有效地将产品与查询相匹配。 低质量的产品数据会导致搜索结果相关性降低。 要直接影响促销策略的成功，请执行以下操作：
  ...
  - 确保这些属性中的数据相关。
- 设计良好的搜索体验为客户建立了信任，并让他们自信地找到了所需的内容。
- 搜索规则至关重要，因为它们可以根据热门程度、新到达次数、促销标准或任何其他促销策略来提高某些产品的可见性，以满足您的业务需求。
- 多面向导航让购物者能够优化搜索并快速获得相关结果。

要了解[!DNL Live Search]如何优先处理&#x200B;**精确匹配和接近匹配、相同字段匹配和跨字段匹配**，以及这些因素如何与搜索权重和规则进行交互，请参阅[搜索匹配和排名](search-relevance-matching.md)。

要管理[!DNL Live Search]，请在Adobe [!DNL Commerce]管理员中转到&#x200B;**营销** > *SEO和搜索* > **[!DNL Live Search]**。 

## 优化搜索功能

在本节中，您将了解如何使用自动完成、同义词和Facet等功能来优化搜索功能。
...
默认情况下，实时搜索会管理错误拼写。 要包含购物者使用的与您目录中指定的单词不同的单词，请设置同义词。 您不希望因为某人正在寻找“沙发”而失去销售，而您的产品却被列为“沙发”。 通过输入客户用于查找产品的所有可能词语，您可以捕获广泛的搜索词。 为了改进结果，[将同义词设置为单向或双向](synonyms-add.md#step-2-define-the-synonym-by-type)。

#### 优化同义词的提示

- 将品牌名称和缩写映射到其全名，例如“HP”映射到“Hewlett-Packard”，将常用产品昵称映射到“iPhone”映射到“Apple iPhone”。
- 包括特定行业的术语和购物者可互换使用的术语，例如“运动鞋”和“跑鞋”。
- 根据新的搜索趋势、产品添加和购物者行为定期更新同义词列表。
- 通过分析搜索结果和购物者反馈来测试同义词映射的有效性。 为了提高准确性和相关性，请优化映射。
...
- 确定产品的最相关属性，并将它们设置为[动态Facet](facets-type.md)。 
...
- 要保持接口干净且可管理（尤其是在移动设备上），请实施可折叠的Facet部分。
...
确保可配置产品的子产品具有具有正确角色的图像。 缺少父产品或子产品导致搜索结果没有图像。

>[!NOTE]
>
>搜索结果中的图像因搜索词而异。 如果搜索词确定子产品更相关，则子产品的图像将替换父产品的图像。
...
- 使用[智能排名](rules-add.md#intelligent-ranking)（例如“为您推荐”、“查看次数最多”等），根据每位购物者的行为定制搜索结果。 要定制购物者行为，必须确保正确实施事件。 对于Luma商家，默认情况下提供事件。 对于Headless或自定义实施，您必须根据特定需求[实施事件](https://developer.adobe.com/commerce/services/shared-services/storefront-events/)。
...
选择哪些属性使其成为可搜索属性对搜索质量有很大影响。 使太多属性成为可搜索属性会降低相关性，并导致意外匹配，即使这样会增加返回的结果数。 本节介绍如何刻意选择可搜索属性，以平衡覆盖范围和相关性。

**推荐的可搜索属性：**

- **产品名称** — 高意图，直接描述产品。
- **主要描述** — 购物者期望匹配的简洁产品详细信息。
- **品牌** — 购物者经常按品牌进行搜索。
- **型号/样式** — 具有明确意图的特定标识符。
- **关键功能** — 重要的区分特征（例如，“防水”、“无线”）。
- **材料/结构** — 用于时尚和家具类别。

**避免使这些属性可搜索：**

- **长描述或规范** — 过多文本会产生杂音和意外匹配。
- **类别路径** — 由于分类术语广泛，可能导致不相关结果。
- **包含混合字符的内部SKU代码** — 对部分匹配项创建误报。
- **管理字段** — 与购物者无关的内部注释或仓库代码。
- **HTML内容或设置标记格式** — 技术内容没有提高相关性。

#### 可搜索属性不正确导致的常见问题

让错误的属性可搜索会让购物者感到沮丧，并造成支持升级。

| 示例 | 方案 | 推荐 |
|---------|----------|----------------|
| **消除和自动完成副作用** | 商家使长产品描述可搜索。 购物者搜索“可以”（查找容器）。 由于词干和部分匹配，描述中大写词包含“can”的产品会出现出来，例如“American”、“canopy”或“canvas”。 与购物者的意图表面无关的产品，他们会失去对搜索的信心。 | 将可搜索字段减少为高意图属性，如产品名称和主要描述。 验证热门搜索词以识别有问题的匹配。 使用搜索促销规则或重定向处理特定的已知边缘情况。 |
| **按人气进行排名会放大杂音匹配** | 商家将排名设置为“购买最多”，并包括类别路径和作为可搜索属性的详细说明。 一位购物者搜索“笔记本电脑包”。 广泛匹配的产品会退回笔记本电脑包、笔记本电脑配件、其他用途的包以及笔记本电脑本身。 由于笔记本电脑比笔记本电脑包更常购买，因此它们位列前列。 即使系统按配置运行，购物者仍认为排名不正确。 | 删除类别路径等杂乱的可搜索属性。 一旦匹配集更精确，则应用基于人气的排名策略。 监视搜索分析，以识别出现此模式的查询。 |
| **类别路径创建误报** | 商家使整个类别路径可搜索（例如，“主页>厨房>家电>小型家电”）。 一位购物者搜索“家庭办公桌”。 “家庭”类别中的产品即使为厨房用品也匹配，因为此类别路径中存在“家庭”类别。 厨房电器、家居装饰和其他不相关的产品出现在结果中，削弱了相关性。 | 请勿使类别路径可搜索；请改用Facet以允许购物者按类别过滤。 如果类别筛选对您的搜索策略至关重要，请通过促销规则而不是可搜索属性来实施它。 |

#### 适当加权可搜索属性

要增加搜索相关性，请为每个可搜索属性分配一个权重。 权重较高的属性在搜索结果中显示得越高。 多个标准（如搜索权重）会影响相关性排序。 这意味着，有时搜索权重较低的属性仍可以比搜索权重较高的属性具有更大的相关性。 其他标准可以包括任何给定属性中的匹配数、找到的搜索词的位置以及搜索词之前和之后的整体文本结构。

**权重优先级：**

- **最高权重(9-10)：**&#x200B;产品名称、品牌
- **Medium权重(6-8)：**&#x200B;型号、主要说明、主要功能
- **较低重量(3-5)：**&#x200B;次要说明、材料、规格
...
如果搜索结果看起来随意或不相关，请在升级为产品缺陷之前使用此核对清单：

1. **查看可搜索属性配置：**

   - 列出当前设置为可搜索的所有属性。
   - 识别任何广泛或杂乱的属性（较长的描述、类别路径、管理字段）。
   - 从不代表购物者意图的属性中删除可搜索状态。

1. **验证查询是否与核心属性匹配：**

   - 测试最常见的搜索查询。
   - 验证结果是否主要与产品名称和品牌匹配，而不是与切向文本匹配。
   - 检查意外结果是否仅在长格式内容中共享弱匹配。

1. **测试是否包含噪音字段：**

   - 从类别路径和详细描述字段中临时删除可搜索状态。
   - 重新运行有问题的查询，以查看相关性是否得到改善。
   - 如果结果改善，请永久调整您的配置。

1. **对例外使用促销规则：**

   - 对于需要特殊处理的特定已知查询，请创建定向搜索规则。
   - 不要尝试通过使更多属性可搜索来解决边缘情况。
   - 使用重定向进行品牌名称搜索或常见拼写错误。

1. **监视和迭代：**

   - 使用[性能工作区](performance.md)跟踪零结果率和点进率。
   - 每周查看热门搜索查询以识别新模式。
   - 根据数据（而非假设）调整可搜索属性和权重。
...
要使用[!DNL Live Search]优化搜索结果，请监视KPI以了解购物者如何与您的搜索功能进行交互。 此数据可指导您定期更新和优化搜索规则。

您可以在[!DNL Live Search] [性能工作区](performance.md)中监视这些KPI，您可以在其中找到以下量度： 

- **唯一搜索** — 在您的[!DNL Commerce]网站上执行的不同搜索查询计数。 系统只计算一次每个唯一搜索，即使同一购物者或不同购物者多次重复搜索也是如此。 此量度可帮助您了解客户使用的搜索词的多样性，并让您深入了解购物者想要的产品或信息。 跟踪唯一搜索允许您：
...
- **点进率(CTR)** — 测量执行搜索查询后点击搜索结果的购物者百分比。 CTR高表明搜索结果与购物者相关，并且很有吸引力，因为他们会单击找到的结果。 监测CTR有助于确定需要改进的方面。 低CTR表示搜索结果与购物者意图不匹配，这提示需要优化搜索规则、增强产品数据或改进结果呈现。

- **转化率** — 表明您的搜索功能在推动销售和实现业务目标方面的有效性。 它反映了搜索功能在满足购物者需求和促进顺畅购物体验方面的整体有效性。 高转化率表示您的搜索结果高度相关且具有说服力，可引导购物者完成购买。 如果转化率很低，则会建议搜索相关性、产品可用性或购物者从搜索到购买的总体旅程中存在的问题。
...
- **常见结果** — 通过使搜索结果与购物者的偏好和行为保持一致，增强搜索结果。

您可以使用此量度数据，并通过以下方式优化搜索功能：

- 要在搜索结果中排名较高的热门产品，请实施规则。 可以优先考虑经常点击或购买的产品，使其显示在顶部。 手动整理特定搜索查询的热门产品列表，并确保突出显示这些项目。
- 突出显示当前热门或最近人气飙升的产品。 此策略在季节性活动、节假日或促销期间生效。 要实现此目的，请在设置搜索规则时，使用更符合您的用例和业务需求的智能排名。
- 突出显示热门过滤器或Facet；如果购物者经常按特定品牌或价格范围进行过滤，请固定这些Facet并相应对其进行排序，以使这些选项更加突出。
- 当搜索产生零结果时，使用热门结果数据来建议具有高购物者参与度的替代产品或相关类别。
- 要识别重要关键词，请分析热门搜索词和产品数据。 为了提高搜索相关性，请使用这些关键字优化产品可搜索属性。
- 定期分析结果数据以了解不断变化的趋势、购物者偏好和行为、识别热门搜索词以及检测问题。 使用此反馈循环不断优化和改进您的搜索规则和产品。

要获取[!DNL Live Search]报表中的正确数据，必须确保正确实施事件。 对于Luma商家，默认情况下提供事件。 对于Headless或自定义实施，您必须根据特定需求[实施事件](https://developer.adobe.com/commerce/services/shared-services/storefront-events/)。
