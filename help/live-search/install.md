...
> **HIPAA准备就绪**
>
> 实时搜索不是HIPAA就绪的服务。 如果您正在将Adobe Commerce与HIPAA-Ready扩展和Healthcare加载项一起使用，请不要在可能处理受保护健康信息(PHI)的任何环境中启用实时搜索。
>
> 有关详细信息，请参阅Adobe Commerce[上的](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/start/compliance/hipaa-ready-service/overview)HIPAA准备就绪[指南和](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/start/compliance/hipaa-ready-service/operations)操作指南，其中列出了未准备好HIPAA的Commerce服务中的实时搜索。
...
使用[数据管理仪表板](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard)监视同步进度。 此仪表板提供关于您店面产品数据的可用性的宝贵见解，确保可以及时向客户显示这些数据。
...
[!DNL Live Search]收集用户交互数据以改进搜索功能并将此信息存储在浏览器Cookie中。 启用Cookie限制时，此数据收集需要用户同意。 [!DNL Live Search]和[!DNL Product Recommendations]共享相同的数据收集机制和Cookie处理。 有关Cookie限制和隐私合规性的详细信息，请参阅[处理Cookie限制](../product-recommendations/setting-cookie.md)。
