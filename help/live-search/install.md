...
使用[数据管理仪表板](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard.md)监视同步进度。 此仪表板提供关于您店面产品数据的可用性的宝贵见解，确保可以及时向客户显示这些数据。
...
默认情况下，如果您未使用搜索适配器、Luma构件或AEM CIF构件等标准组件，则自动收集用户交互数据将不起作用。 Adobe AI将收集的数据用于智能推销和性能跟踪。 要解决此问题，您需要开发自定义解决方案，以采用Headless方式实施此数据收集。
