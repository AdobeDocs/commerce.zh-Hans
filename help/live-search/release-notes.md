...
![New](../assets/new.svg) [!DNL Live Search]现在支持语义搜索，可从&#x200B;[**设置**](./settings.md#semantic-search)&#x200B;工作区进行配置。 与关键词搜索不同，语义搜索使用人工智能根据含义和上下文匹配产品 — 帮助减少自然语言查询（如“冬季徒步的温暖”）的零结果搜索。 [了解详情](./semantic-search.md)。
...
![新](../assets/new.svg)Commerce管理员现在可以为[!DNL Live Search]指定索引的语言。请参阅[设置](settings.md)。
...
针对以下高级搜索功能的![新](../assets/new.svg) GA版本：使用`startsWith`和`contains`的分层搜索。[了解更多](workspace.md#layered-search-and-expansion-of-search-types)。
...
![新](../assets/new.svg)类别预览仅显示所选类别。
![新](../assets/new.svg) AEM CIF [Popover构件](https://github.com/adobe/aem-cif-guides-venia/pull/319)和[PLP构件](https://github.com/adobe/aem-cif-guides-venia/pull/320)组件允许AEM站点利用[!DNL Live Search]。
...
- 14天
- 30天
![修复](../assets/fix.svg)“事件”（提升/固定/伯里/隐藏）已重命名为“手动排名”
...
![Fix](../assets/fix.svg) `categories`和`categoryPath`已替换`categoryIds`进行类别筛选。有关详细信息，请参阅[productSearch](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search/)主题。
...
![新](../assets/new.svg)可以从&#x200B;*管理员*&#x200B;配置[[!DNL storefront popover]](overview.md)中的可用行数。
[!DNL Live Search]支持![新](../assets/new.svg) Beta [PWA](https://developer.adobe.com/commerce/pwa-studio/)。
![新](../assets/new.svg) [!DNL Live Search]安装过程已更新，其中包含高级过程更改。
![修复](../assets/fix.svg) [高级搜索](https://experienceleague.adobe.com/en/docs/commerce-admin/catalog/catalog/search/search)链接已从店面页脚中删除。
...
![错误](../assets/bug.svg) PWA的[!DNL Live Search]测试版不支持[事件处理](https://developer.adobe.com/commerce/services/shared-services/storefront-events/sdk/)。
...
![修复](../assets/fix.svg) [[!DNL Storefront popover]](storefront-popover.md)现在显示[特价](https://experienceleague.adobe.com/en/docs/commerce-admin/catalog/products/pricing/product-price-special)（最低最终价格）（可用时）。
...
![新](../assets/new.svg) [!DNL Live Search] [Storefront事件SDK](https://developer.adobe.com/commerce/services/shared-services/storefront-events/sdk/)提供对公共数据层的访问权限，该数据层包含事件发布和订阅服务以及指标。
![修复](../assets/fix.svg) [[!DNL Storefront popover]](storefront-popover.md)具有用于控制可见性的`.search-autocomplete`容器的新`active`类。
![修复](../assets/fix.svg)在店面，[搜索词](https://experienceleague.adobe.com/en/docs/commerce-admin/catalog/catalog/search/search-terms)页脚链接已被删除，并且其缓存已禁用[!DNL Live Search]安装。
搜索适配器的![错误](../assets/bug.svg)修补程序处理重复的产品。
![错误](../assets/bug.svg) [!DNL Live Search]支持具有多个（虚拟）[库存](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/stocks/stocks-manage)的[单个源](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/sources/sources-manage)（物理）库存位置。现在不支持多个清单源。
...
![修复](../assets/fix.svg)增量产品更新近乎实时（安装和设置后）
...
![错误](../assets/bug.svg)添加Facet时，产品属性信息源在设置为`Update on Save`时未正确更新。若要避免出现此问题，请转到[索引管理](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/tools/index-management)并将产品属性信息源设置为`Update by Schedule`。
